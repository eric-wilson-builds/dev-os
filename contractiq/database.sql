...
-- Paste this entire file into the Supabase SQL Editor (or run via the Supabase
-- CLI / psql) and execute it once on a fresh project. Every statement is
-- idempotent, so it's also safe to re-run after future edits to this file.
--
-- Source of truth: docs/engineering/engineering-doc.md (Section 7 — Database
-- Design and Schema) and docs/specs/*.md (per-feature DB requirements). This
-- file mirrors docs/specs/supabase-schema.sql exactly — that's the spec
-- deliverable, this is the copy you actually run. Keep the two in sync.
-- =============================================================================
...
-- Backs the token-bucket rate limiter for /process and /chat (see
-- docs/specs/infrastructure-spec.md). No extra service (e.g. Redis) required —
-- kept in Postgres to minimise ops surface.
-- -----------------------------------------------------------------------------
...
-- manually by an operator after reviewing a scripts/eval/ report). RLS is
-- enabled with zero policies (default-deny) — only the service role can read
-- or write it; see docs/specs/results-display-spec.md's calibration banner.
-- -----------------------------------------------------------------------------
